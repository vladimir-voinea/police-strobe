...
police_lights::police_lights() : m_red({2}, {3}, {4}), m_blue({5}, {6}, {7})
{
}
...
void police_lights::transition_if_needed()
{
    ...
    static auto go_to_next_state = [this, state_changes = 0]() mutable {
        m_state = static_cast<police_lights_state>((static_cast<int>(m_state) + 1) %
                                                   static_cast<char>(police_lights_state::STATE_COUNT));
        m_cycles_for_state = 0;
        state_changes++;
...
struct blink
{
    blink(int duration_ms) : duration_ms(duration_ms)
    {
    }
...
template <typename T> void blink_all(int duration_ms, T &leds)
{
    auto turn_on = [](led &led) { led.turn_on(); };

    auto turn_off = [](led &led) { led.turn_off(); };
...
void police_lights::loop()
{
    ...
    [[maybe_unused]] auto turn_on = [](led &led) { led.turn_on(); };
